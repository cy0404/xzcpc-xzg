# -*- coding: utf-8 -*-
"""8/7 周盘理论剩余计算 — 口径与 SmartOrderServiceImpl 完全一致"""
...
for mid, s in summaries.items():
    ...
    negative = theo < 0
    if negative: theo = 0.0  # 库存不为负，截断
    rows.append((mid, s["material_name"], cat, specs.get(mid, ""), end_qty, pur, sales, lossv, trv, rtv, spv, theo, excluded, negative))

rows.sort(key=lambda x: (x[12], x[0]))
print(f"\n{'material_id':<14}{'名称':<18}{'期末':>9}{'入库':>8}{'销量':>9}{'报损':>8}{'调净':>8}{'还净':>8}{'自购':>8}{'理论剩余':>10}")
for r in rows:
...
lines = []
lines.append("-- 8/7 周盘测试任务（理论剩余自动计算，2026-08-20 生成）")
lines.append("-- 库: store_inventory_test  门店: 象子茶铺茶怒江东岸人民路店 (MDCK000082)")
lines.append("-- 口径: 理论剩余 = 594期末(7/31) + 8/1~8/7入库 − 6天消耗(PG销量+报损+调货+还货−自购)")
...
lines.append(f"VALUES ('{STORE_ID}', '{store_name}', '{store_code}', '{xcx}', '{wh}', NULL, '8/7周盘测试任务', '2026-08', 'weekly', '2026-W32', '2026-08-07 12:00:00', 'submitted', 'system', '陈燕', '2026-08-07 12:00:00', '{biz}', 0);")
...
vals = []
for r in rows:
    mid, name, cat, spec, end_qty, pur, sales, lossv, trv, rtv, spv, theo, excluded, negative = r
    rule = rules.get(mid) or {}
    base = rule.get("base_unit") or "g"
    vals.append(f"(@task_id, '{mid}', '{name}', '{spec}', '{base}', {theo:.2f}, {theo:.2f}, {theo:.2f}, 0, NULL, 0, '', 0)")
lines.append(",\n".join(vals) + ";")
lines.append("-- 执行后请回传任务ID（SELECT LAST_INSERT_ID() 或查最新 TASK id），用于 backtest")
sql_path = r"C:\Users\xiemg\Documents\test\inventory-tool\tmp_weekly_task_0807.sql"
