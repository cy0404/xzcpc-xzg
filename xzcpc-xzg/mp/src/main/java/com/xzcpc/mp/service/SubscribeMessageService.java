 ...
 *
 * 模板字段约定（已申请模板「工作任务待办通知」#49925，字段 key 按关键词位置编号）：
 *   thing1 = 标题/待办类型（如【报损审批】今日有 3 条报损待审批）
 *   thing2 = 说明内容/待办内容
 *   time3  = 时间（第 3 个关键词为 time 类型 → key 为 time3）
 * 若日后换模板，按实际字段名调整下方 data key。
 *
        ...
        try {
            WxMaSubscribeMessage msg = WxMaSubscribeMessage.builder()
                    ...
                    .data(List.of(
                            ...
                            new MsgData("time3", LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")))))
                    .build();
